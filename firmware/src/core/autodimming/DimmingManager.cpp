...
namespace {
    ...
    constexpr uint16_t temp = ((NUM_BRIGHTNESS_LEVELS < lightDiscreteValues) ? NUM_BRIGHTNESS_LEVELS : lightDiscreteValues);
    constexpr uint16_t numDimmingLevels = ((temp < brightnessDiscreteValues) ? temp : brightnessDiscreteValues);
...
DimmingManager *DimmingManager::m_instance = nullptr;
...
void DimmingManager::updateBrightness(ScreenManager *sm, WidgetSet *ws) {
    ...
    int16_t filtered = filter(lightIntensity);

    //    Log.infoln("Dimming Lgt:%d Filt:%d Lo:%d Hi:%d Lvl:%d Brt:%d\n",
    ...
    int16_t level = hysteresis(filtered);
    if (level < 0) {
        return;
    }
    brightnessLevel_ = static_cast<uint16_t>(level);

    // Compute new brightness value in the interval [AUTO_BRIGHTNESS_MIN, AUTO_BRIGHTNESS_MAX]
    screenBrightness_ = (brightnessLevel_ * brightnessStepSize) + AUTO_BRIGHTNESS_MIN;

    // Update the display
    sm->setBrightness(screenBrightness_);
    ws->drawCurrent(true);
}
...
bool DimmingManager::isTimeToUpdate() {
    ...
    uint64_t elapsedTime_uS = currentTime_uS - lastUpdateTime_uS_;
...
uint16_t DimmingManager::filter(uint16_t newLightIntensity) {
    ...
    lightIntensity_ = static_cast<uint16_t>((alpha * lightIntensity_) +
...
int16_t DimmingManager::hysteresis(uint16_t newLightIntensity) {
    if (newLightIntensity >= lowerHysteresis_ && newLightIntensity <= upperHysteresis_) {
    ...
    lowerHysteresis_ = (boundary > tolerance) ? boundary - tolerance : 0;
    upperHysteresis_ = boundary + lightStepSize + tolerance;
