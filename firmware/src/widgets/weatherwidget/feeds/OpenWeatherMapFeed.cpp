...
OpenWeatherMapFeed::OpenWeatherMapFeed(const String &apiKey, const float &lat_id, const float &lon_id, const std::string &location)
    : apiKey(apiKey), lat_id(lat_id), lon_id(lon_id), m_weatherLocation(location) {}
...
bool OpenWeatherMapFeed::getWeatherData(WeatherDataModel &model) {
    ...
    String httpRequestAddress = String(WEATHER_OPENWEATHERMAP_API_URL) + "?lat=" + String(lat_id) + "&lon=" + String(lon_id) +
